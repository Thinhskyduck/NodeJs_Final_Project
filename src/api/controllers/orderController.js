...
const createOrder = async (req, res) => {
  ...
  try {
    ...
    // 4. Tính tổng tiền ban đầu (Trước khi giảm)
    ...
    // Biến theo dõi giảm giá
    let discountAmount = 0; // Tiền giảm từ Coupon
    let loyaltyDiscount = 0; // Tiền giảm từ Điểm
    let actualPointsUsed = 0; // Số điểm thực tế bị trừ

    // 5. Xử lý Coupon (Ưu tiên trừ Coupon trước)
    ...
    if (discountCode) {
      ...
      if (discount && discount.timesUsed < discount.maxUses) {
        // Logic kiểm tra ngày hết hạn, min order value... (tự thêm nếu cần)
    ...
    // 6. Xử lý điểm Loyalty (Theo logic bạn gợi ý + Tối ưu)
    // Chuyển đổi pointsToUse sang số để tránh lỗi logic
    ...
    // 7. Cộng điểm thưởng cho đơn hàng mới
    // LƯU Ý: Chỉ nên cộng điểm trên số tiền thực trả (totalPrice sau khi giảm)
    // Nếu totalPrice = 0 thì không được cộng điểm.
    const pointsEarned = Math.floor(totalPrice / 10000); // 10k = 1 điểm
    ...
    const order = new Order({
        ...
        // Lưu thông tin Điểm (Cần thêm field vào Model Order nếu chưa có)
